...
const ACFTechnicalCapabilities = ({ pageSlug = 'homepage' }) => {
    ...
    return (
      <section className="py-20 bg-tactical-gray flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-300">Loading Technical Capabilities section...</p>
        </div>
      </section>
    )
  }
  ...
  if (error || !content) {
    return <FallbackTechnicalCapabilities />
  }

  // Extract ACF data
  const acf = content.acf || {}
  const {
    // Technical Capabilities Header
    tech_capabilities_title_line1 = 'OPERATIONAL',
    tech_capabilities_title_line2 = 'CAPABILITIES',
    tech_capabilities_description = 'Full-spectrum technology services engineered for organizations that demand excellence, reliability, and results that exceed expectations'
  } = acf

  // Process Core Capabilities from ACF flat structure
  const processCoreCapabilities = (acfData) => {
    const capabilities = []
    let capIndex = 0
    
    while (acfData[`core_capabilities_${capIndex}_title`]) {
      const capability = {
        icon: acfData[`core_capabilities_${capIndex}_icon`],
        iconColor: acfData[`core_capabilities_${capIndex}_icon_color`],
        iconBg: acfData[`core_capabilities_${capIndex}_icon_bg`],
        hoverBorder: acfData[`core_capabilities_${capIndex}_hover_border`],
        title: acfData[`core_capabilities_${capIndex}_title`],
        description: acfData[`core_capabilities_${capIndex}_description`],
        buttonText: acfData[`core_capabilities_${capIndex}_button_text`],
        buttonClass: acfData[`core_capabilities_${capIndex}_button_class`],
        features: []
      }
      
      // Process features for this capability
      let featureIndex = 0
      while (acfData[`core_capabilities_${capIndex}_features_${featureIndex}_text`]) {
        capability.features.push({
          icon: acfData[`core_capabilities_${capIndex}_features_${featureIndex}_icon`],
          text: acfData[`core_capabilities_${capIndex}_features_${featureIndex}_text`]
        })
        featureIndex++
      }
  ...
  // Process Specialized Services from ACF flat structure
  const processSpecializedServices = (acfData) => {
    const services = []
    let serviceIndex = 0
    
    while (acfData[`specialized_services_${serviceIndex}_title`]) {
      const service = {
        icon: acfData[`specialized_services_${serviceIndex}_icon`],
        iconColor: acfData[`specialized_services_${serviceIndex}_icon_color`],
        iconBg: acfData[`specialized_services_${serviceIndex}_icon_bg`],
        title: acfData[`specialized_services_${serviceIndex}_title`],
        subtitle: acfData[`specialized_services_${serviceIndex}_subtitle`],
        description: acfData[`specialized_services_${serviceIndex}_description`],
        buttonText: acfData[`specialized_services_${serviceIndex}_button_text`],
        buttonClass: acfData[`specialized_services_${serviceIndex}_button_class`],
        stats: []
      }
      
      // Process stats for this service
      let statIndex = 0
      while (acfData[`specialized_services_${serviceIndex}_stats_${statIndex}_value`]) {
        service.stats.push({
          value: acfData[`specialized_services_${serviceIndex}_stats_${statIndex}_value`],
          label: acfData[`specialized_services_${serviceIndex}_stats_${statIndex}_label`],
          color: acfData[`specialized_services_${serviceIndex}_stats_${statIndex}_color`]
        })
        statIndex++
      }
  ...
  const acfCoreCapabilities = processCoreCapabilities(acf)
  const acfSpecializedServices = processSpecializedServices(acf)
  ...
  const defaultSpecializedServices = [
    {
  ...
  const displayCoreCapabilities = acfCoreCapabilities.length > 0 ? acfCoreCapabilities : defaultCoreCapabilities
  const displaySpecializedServices = acfSpecializedServices.length > 0 ? acfSpecializedServices : defaultSpecializedServices
      ...
      <div className="container mx-auto px-6">
        ...
        <div className="text-center mb-16">
          ...
          <h2 className="font-display font-black text-5xl lg:text-6xl mb-6">
            <span className="text-white">{tech_capabilities_title_line1}</span>
            <span className="text-fire-orange"> {tech_capabilities_title_line2}</span>
          </h2>
          <p className="text-xl text-gray-300 max-w-4xl mx-auto">
            {tech_capabilities_description}
          </p>
        </div>

        {/* Core Capabilities - 3 Column Grid */}
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-16">
          {displayCoreCapabilities.map((capability, index) => (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          {displaySpecializedServices.map((service, index) => (
...
const FallbackTechnicalCapabilities = () => {
      ...
      <div className="container mx-auto px-6">
        <div className="bg-yellow-900/20 border border-yellow-600 rounded-lg p-4 max-w-2xl mx-auto mb-8">
          <p className="text-yellow-400 text-center text-sm">
            ...
            Using fallback Technical Capabilities content - WordPress not connected
          </p>
        </div>
...
export default ACFTechnicalCapabilities